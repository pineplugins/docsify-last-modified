...
window.$docsify.plugins.push((hook, vm) => {
  hook.afterEach((html, next) => {
    ...
    const gitSha = window.GITHUB_SHA || Date.now();
    const metaUrl = `plugin/meta/${slug}.json?v=${gitSha}`;
    ...
    fetch(metaUrl)
      ...
      .then(meta => {
        ...
        // Pasang event handler langsung setelah next dipanggil supaya elemen sudah di-render
        const openRawMeta = document.getElementById('openRawMeta');
        if (openRawMeta) {
          openRawMeta.addEventListener('click', () => {
            const newWindow = window.open('', '_blank', 'width=600,height=400,scrollbars=yes,resizable=yes');
            newWindow.document.write(
              `<pre style="white-space: pre-wrap; font-family: monospace; font-size: 14px;">${JSON.stringify(meta, null, 2)}</pre>`
            );
